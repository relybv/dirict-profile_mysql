#!/usr/bin/env bash
#
# This script installs puppet 3.x or 4.x and deploy the manifest using puppet apply -e "include profile_mysql"
#
# Usage:
# Ubuntu / Debian: wget https://raw.githubusercontent.com/relybv/dirict-profile_mysql/master/files/bootme.sh; bash bootme.sh
#
# Red Hat / CentOS: curl https://raw.githubusercontent.com/relybv/dirict-profile_mysql/master/skeleton/files/bootme.sh -o bootme.sh; bash bootme.sh
...
if which puppet > /dev/null 2>&1; then
    ...
    bash /root/profile_mysql/files/bootstrap.sh $PUPPETMAJOR
fi
